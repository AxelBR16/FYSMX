...
if ($id == ''  || $token == '') {
    ...
    if($token_tmp == $token) {
     ...
     //Funcion Sedes
        ...
        function sedes($sedes){      
            $colonia = [];
            $direccion = '';

                $datosAreas = explode('*', $sedes);
                for ($i = 1; $i < count($datosAreas); $i += 2) {
                    $area = trim($datosAreas[$i]);
                    if (!in_array($area, $colonia)) {
                        if(!empty($direccion)){
                            echo"";
                        }
                        echo "<h4>$area</h4>";
                    ...
                    if ($i + 1 < count($datosAreas)) {
                        $direccion = trim($datosAreas[$i + 1]);
                        // Dividir las direccion por " y crear <li> para cada una
                        $direccionArray = explode('"', $direccion);
                        foreach ($direccionArray as $dir) {
                            echo "<p>$dir</p>";
                        }
                    }
    ...
    //Funcion Plan de Estudios
        ...
        }//Fin funcion plan de studios

    //Proceso de Admision 
        function admi($procesoAdmision){        
            $prin = [];
            $secun = '';

                $datosAreas = explode('*', $procesoAdmision);
                // Iterar sobre las áreas encontradas
                for ($i = 1; $i < count($datosAreas); $i += 2) {
                    $area = trim($datosAreas[$i]);

                    if (!in_array($area, $prin)) {
                        //cerrar el ul anterior si existe
                        if(!empty($secun)){
                            echo"</ul>";
                        }
                        // Imprimir el encabezado <h4> para el área
                    ...
                    if ($i + 1 < count($datosAreas)) {
                        $secun = trim($datosAreas[$i + 1]);
                        // Dividir las secun por " y crear <li> para cada una
                        $pasosArray = explode('"', $secun);
                ...
                <img  class="img-fluid detalles__imagen" src="<?php echo $imagen; ?>" alt="">
                    ...
                    <div class="proadmi">
                        ...
                        <div class="proadmi_con">
                            ...
                            <!--<img src="./img/proadmi.png" alt="Imagen planest"> -->
                        </div>
